        ...
        <div className="absolute inset-0 flex items-center justify-center bg-background">
          <div className="flex flex-col items-center gap-3">
            ...
            <span className="text-[11px] tabular-nums text-muted-foreground/70">
              {pct > 0 ? `${Math.round(pct)}%` : (fallbackLoading ? "Retrying…" : "Loading PDF…")}
            </span>
          </div>
        </div>
      );
      ...
      >
        ...
        {file && (
          <Document
            ...
            loading={
              <div className="flex min-h-[60vh] items-center justify-center px-8 motion-safe:animate-[fade-in_180ms_ease-out_120ms_both]">
                <div className="flex flex-col items-center gap-3">
                  <SpokeSpinner size={28} ariaLabel={fallbackLoading ? "Retrying" : "Loading PDF"} />
                  <div
                    className="h-1 w-40 overflow-hidden rounded-full bg-muted"
                    role="progressbar"
                    aria-valuemin={0}
                    aria-valuemax={100}
                    aria-valuenow={progress && progress > 0 ? progress : undefined}
                    aria-label="PDF loading progress"
                  >
                    <div
                      className={`h-full bg-primary transition-[width] duration-200 ease-out ${progress && progress > 0 ? "" : "animate-pulse"}`}
                      style={{ width: `${progress && progress > 0 ? progress : 12}%` }}
                    />
                  </div>
                  <span className="text-[11px] tabular-nums text-muted-foreground/80">
                    {progress && progress > 0
                      ? `${Math.round(progress)}% • Loading PDF…`
                      : fallbackLoading
                        ? "Stabilizing stream…"
                        : "Preparing PDF…"}
                  </span>
