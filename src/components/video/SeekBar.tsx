...
const SeekBar = memo(({
  ...
  return (
    ...
    >
      {/* Visible track */}
      <div className="absolute left-0 right-0 top-1/2 -translate-y-1/2 h-1.5 group-hover/seek:h-2.5 md:group-hover/seek:h-3 rounded-full bg-player-on-surface/15 transition-[height] duration-150 motion-reduce:transition-none overflow-visible">
        {/* Buffered */}
        <div
          className="absolute inset-y-0 left-0 bg-player-on-surface/35 rounded-full pointer-events-none"
        ...
        {markerData.ch.map((c) => {
          ...
          return (
            <div
              key={c.id}
              aria-hidden
              className="absolute top-1/2 -translate-y-1/2 w-px h-2.5 bg-player-on-surface/70 pointer-events-none"
              style={{ left: `${left}%` }}
            />
          );
        })}
      ...
      {tooltipTime !== null && (
        <div
          className="absolute -top-9 left-0 px-2 py-1 rounded bg-player-surface/90 text-player-on-surface text-xs whitespace-nowrap pointer-events-none -translate-x-1/2"
