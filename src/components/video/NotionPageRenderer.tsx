...
export default function NotionPageRenderer({ url, title, onClose, onReady, onDocument }: Props) {
  ...
  useEffect(() => () => {
    ...
    if (pendingBlobUrlRef.current) {
      try { URL.revokeObjectURL(pendingBlobUrlRef.current); } catch { /* best-effort cleanup: revoke is idempotent */ }
  ...
  const popOrClose = useCallback(() => {
    setStack((s) => {
      ...
      if (onClose) onClose();
      else try { window.history.back(); } catch (e) { reportError(e, { surface: "notion-renderer", op: "history-back" }); }
      return s;
    });
  }, [onClose]);
    ...
    try {
      ...
      try {
        ...
        revokeTimerRef.current = setTimeout(() => {
          try { URL.revokeObjectURL(blobUrl); } catch { /* best-effort cleanup: revoke is idempotent */ }
  ...
  useEffect(() => {
    ...
    const onPop = (e: PopStateEvent) => {
      ...
      try { window.history.pushState({ pdfFullscreen: true }, ""); } catch (e) { reportError(e, { surface: "notion-renderer", op: "pushState-fullscreen" }); }
    };
  ...
  useEffect(() => {
    if (!pageId) {
      ...
      try { window.dispatchEvent(new CustomEvent("pdf-error", { detail: "Could not extract Notion page id." })); } catch (e) { reportError(e, { surface: "notion-renderer", op: "dispatch-pdf-error" }); }
      return;
    }
    let cancelled = false;
    ...
    const fetchOnce = async (): Promise<unknown> => {
      ...
      if (!res.ok) {
        ...
        } catch (e) { reportError(e, { surface: "notion-renderer", op: "parse-error-body" }); }
        ...
        if (TRANSIENT.test(msg)) {
          ...
          try {
            ...
            if (!isExpectedNotionAccessError(e2)) {
              try { window.dispatchEvent(new CustomEvent("pdf-error", { detail: msg2 })); } catch (e) { reportError(e, { surface: "notion-renderer", op: "dispatch-pdf-error" }); }
            }
            return;
          }
        } else {
          ...
          if (!isExpectedNotionAccessError(e)) {
            try { window.dispatchEvent(new CustomEvent("pdf-error", { detail: friendly })); } catch (e) { reportError(e, { surface: "notion-renderer", op: "dispatch-pdf-error" }); }
          }
          return;
        }
      }
  ...
  useEffect(() => {
    ...
    const id = requestAnimationFrame(() => {
      try { window.dispatchEvent(new CustomEvent("pdf-ready")); } catch (e) { reportError(e, { surface: "notion-renderer", op: "dispatch-pdf-ready" }); }
              ...
              <a
                ...
                onClick={(e) => {
                  ...
                  if (typeof href === "string" && href) {
                    ...
                    setStack((s) => [...s, next]);
                    try { window.history.pushState({ pdfFullscreen: true, notionSub: true }, ""); } catch (e) { reportError(e, { surface: "notion-renderer", op: "pushState-subpage" }); }
                  }
                }}
