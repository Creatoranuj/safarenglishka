...
export class PlayerErrorBoundary extends Component<Props, State> {
  ...
  render() {
    ...
    return (
      <div
        role="alert"
        className="relative aspect-video w-full bg-player-surface rounded-xl overflow-hidden flex flex-col items-center justify-center gap-3 p-4 text-center"
      >
        <AlertTriangle className="h-8 w-8 text-amber-400" aria-hidden />
        <div className="text-player-on-surface/90 text-sm font-medium">
          Player couldn't load this video
        </div>
        {this.state.message ? (
          <div className="text-player-on-surface/50 text-xs max-w-[80%] line-clamp-2">
            {this.state.message}
          </div>
        ) : null}
        <button
          ...
          className="mt-1 inline-flex items-center gap-2 px-4 py-2 rounded-md bg-player-on-surface/10 hover:bg-player-on-surface/20 active:bg-player-on-surface/30 text-player-on-surface text-sm transition-colors"
        >
