 ...
 * 
 * Features:
 * 1. Full "Safar English" watermark covering YouTube logo (bottom-right)
...
const MahimaVideoPlayer: React.FC<MahimaVideoPlayerProps> = ({ videoUrl, onEnded, onReady, onDurationReady }) => {
  ...
  if (!videoUrl) {
    return (
      <div className="aspect-video bg-player-surface rounded-xl flex items-center justify-center">
        <p className="text-player-on-surface/50">No video URL provided</p>
      </div>
    );
  }
  ...
  return (
    <div
      ref={wrapperRef}
      className={cn(
        "relative w-full bg-player-surface rounded-xl overflow-hidden select-none group",
    ...
    >
      ...
      <div className={cn("relative", isFullscreen ? "h-full w-full" : "aspect-video")}>
        ...
        {(!isReady || isBuffering) && !hasError && (
          <div className="absolute inset-0 flex items-center justify-center bg-player-surface/80 z-20">
            <div className="text-center">
              <Loader2 className="w-12 h-12 text-player-on-surface animate-spin mx-auto mb-3" />
              <p className="text-player-on-surface/70 text-sm">{isBuffering ? 'Buffering...' : 'Loading video...'}</p>
            </div>
          </div>
        )}
        ...
        {/* Error Overlay */}
        {hasError && (
          <div className="absolute inset-0 flex items-center justify-center bg-player-surface/90 z-20">
            <div className="text-center px-6">
              <div className="w-16 h-16 rounded-full bg-muted/20 flex items-center justify-center mx-auto mb-4">
                <Play className="w-8 h-8 text-muted-foreground" />
              </div>
              <p className="text-player-on-surface font-medium mb-2">Video unavailable</p>
              <p className="text-player-on-surface/50 text-sm">Unable to load this video. Please try again later.</p>
            </div>
          </div>
        )}
        ...
        {!playing && isReady && !hasError && (
          <div 
            className="absolute inset-0 flex flex-row items-center justify-center z-20 cursor-pointer bg-player-surface/60 backdrop-blur-sm"
      ...
      >
        ...
        >
          ...
          <div className="absolute left-0 right-0 top-1/2 -translate-y-1/2 h-1 bg-player-on-surface/20 rounded-full" />
          <div 
            className="absolute top-1/2 -translate-y-1/2 h-1 left-0 bg-player-on-surface/40 rounded-full"
        ...
        <div className="relative flex items-center gap-2 px-4 py-3">
          ...
          <button
            ...
            className="p-2 rounded-full hover:bg-player-on-surface/10 transition-colors"
            disabled={!isReady || hasError}
          >
            {playing ? (
              <Pause className="w-5 h-5 text-player-on-surface" fill="white" />
            ) : (
              <Play className="w-5 h-5 text-player-on-surface ml-0.5" fill="white" />
            )}
          </button>
          ...
          <div className="relative flex items-center">
            ...
            >
              {muted || volume === 0 ? (
                <VolumeX className="w-5 h-5 text-player-on-surface" />
              ) : (
                <Volume2 className="w-5 h-5 text-player-on-surface" />
              )}
            </button>
          ...
          {/* Time display */}
          <div className="text-player-on-surface text-sm font-medium ml-2">
            {formatTime(played * duration)} / {formatTime(duration)}
          </div>

          {/* Quality badge */}
          <div className="px-2 py-1 bg-player-on-surface/10 rounded text-xs text-player-on-surface font-medium hidden sm:block">
            Auto / HD
          </div>
          ...
          <div className="relative">
            <button
              ...
              className="p-3 rounded-full hover:bg-player-on-surface/10 transition-colors flex items-center gap-1 min-h-12 min-w-12 justify-center"
            ...
            {showSettings && (
              <div className="absolute bottom-full right-0 mb-2 bg-player-surface/40 backdrop-blur-xl border border-white/15 rounded-2xl py-1.5 min-w-[110px] shadow-2xl ring-1 ring-white/5 overflow-hidden animate-in fade-in slide-in-from-bottom-1 duration-150">
                <div className="px-3 py-1 text-[10px] uppercase tracking-wider text-player-on-surface/60 font-medium">
                  Speed
                </div>
                  ...
                  {playbackRates.map((rate) => (
                    <button
                      ...
                      className={cn(
                        ...
                        "[@media(hover:hover)]:hover:bg-player-on-surface/10 active:bg-player-on-surface/15",
                        rate === playbackRate ? "text-primary font-semibold" : "text-player-on-surface/90"
                      )}
                    >
                      {rate}x{rate === 1 && <span className="text-player-on-surface/50 text-xs ml-1">Normal</span>}
                    </button>
                  ))}
                </div>
              </div>
          ...
          <button
            ...
            className="p-3 rounded-full hover:bg-player-on-surface/10 transition-colors min-h-12 min-w-12 flex items-center justify-center"
          >
            <PlayerIcon
              kind="rotate"
