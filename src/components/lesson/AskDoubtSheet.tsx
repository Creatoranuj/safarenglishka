...
export const AskDoubtSheet = ({
  ...
  const [local, setLocal] = useState<LocalState>({ likes: {}, liked: {}, feedback: {} });
  ...
  const handleSubmit = () => {
    ...
    if (!/^\d{1,2}:\d{2}\s*[-–—]/.test(raw)) {
      const stamp = formatStamp(getVideoTime());
      setChatInput(`${stamp} - ${raw}`);
      // Defer send to next tick so state updates flush
      ...
      >
        <div className="flex items-center gap-3 px-4 pb-3">
          <button
            ...
            className="h-9 w-9 rounded-full inline-flex items-center justify-center text-foreground hover:bg-accent/40 transition-colors"
          >
        ...
        {threads.length === 0 && !chatBusy && (
          <div className="h-full flex flex-col items-center justify-center text-center px-6 pt-10">
            ...
            <button
              ...
              className="mt-5 inline-flex items-center gap-2 px-5 h-10 rounded-full bg-gradient-to-r from-primary to-primary/80 text-primary-foreground font-semibold text-sm shadow-md hover:shadow-lg transition-shadow"
            >
              ...
              <div>
                ...
                <div className="mt-3 flex items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    aria-label="Report"
                    className="text-muted-foreground hover:text-destructive p-1.5 rounded-full hover:bg-destructive/5"
                  >
            ...
            <div className="rounded-2xl border border-border bg-muted/30 px-3 pt-2 pb-2 shadow-inner">
              ...
              <div className="flex items-center justify-between pt-1">
                <button
                  ...
                  aria-label="Attach"
                  className="h-8 w-8 rounded-lg flex items-center justify-center text-foreground/40"
                >
                ...
                <button
                  ...
                  className="h-8 w-8 rounded-full inline-flex items-center justify-center bg-foreground text-background hover:bg-foreground/90 disabled:bg-muted disabled:text-muted-foreground transition-colors duration-150"
                >
                  {chatBusy ? (
