...
const menuItems: MenuItem[] = [
  ...
  { icon: GraduationCap, labelKey: "nav.myCourses", path: "/my-courses" },
  { icon: BookOpen, labelKey: "nav.courses", path: "/courses" },
  { icon: Library, labelKey: "nav.books", path: "/books" },
  { icon: Download, labelKey: "nav.downloads", path: "/downloads" },
  { icon: Video, labelKey: "nav.doubts", path: "/doubts" },

  { icon: Bell, labelKey: "nav.notices", path: "/notices" },
  { icon: Users, labelKey: "nav.community", path: "/community" },
  { icon: Users, labelKey: "nav.students", path: "/students", adminOrTeacher: true },
  { icon: Calendar, labelKey: "nav.attendance", path: "/attendance", adminOrTeacher: true },
...
const Sidebar = ({ isOpen, onClose }: SidebarProps) => {
  ...
  useEffect(() => {
    ...
    if (!isOpen) {
      // Radix (language sheet) also touches body scroll while the sidebar is
      // open. Always hand the lock back explicitly so the page can never stay
      // frozen after both layers close.
    ...
    <>
      ...
      >
        ...
        <div className="relative flex-1 min-h-0">
          {/* Scroll affordance — top/bottom fades so a clipped row reads as "scroll me" */}
          <div className="pointer-events-none absolute inset-x-0 top-0 h-4 z-10 bg-gradient-to-b from-sidebar to-transparent" aria-hidden="true" />
          <div className="pointer-events-none absolute inset-x-0 bottom-0 h-4 z-10 bg-gradient-to-t from-sidebar to-transparent" aria-hidden="true" />
        <nav className="h-full py-4 overflow-y-auto overscroll-contain [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          {isAuthenticated && isAdmin && (
            <>
              ...
              >
                ...
                <span className="font-medium">{t("nav.chatbot")}</span>
              </Link>
            </>
          )}

          {/* Language switch — lives inside the scrollable nav so the footer stays compact */}
          <div className="mt-3 pt-3 mx-2 border-t border-sidebar-border/60">
        ...
        <div className="p-4 pb-safe-b border-t border-sidebar-border">
          {isAuthenticated && user && (
            <div className="mb-3 px-2">
              <p className="text-sm font-medium text-sidebar-foreground truncate">{profile?.fullName || t("auth.user")}</p>
