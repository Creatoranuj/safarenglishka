...
import { X, ArrowUp, RotateCcw, ThumbsUp, ThumbsDown, Mic, MicOff, Paperclip, Lock, LogIn, Menu, ChevronDown, Plus, MoreHorizontal, Reply, Copy } from "lucide-react";
        ...
        <div className={cn(
          ...
          <div className="safe-area-top bg-background shrink-0">
            <div className="flex items-center justify-between gap-2 px-3 py-2.5">
              ...
              <button
                type="button"
                className="inline-flex items-center gap-1.5 rounded-full border border-border/60 bg-background px-3 h-9 text-sm font-medium text-foreground hover:bg-muted/40 transition-colors"
                aria-label="Safar Agent model"
              >
          ...
          <div className="bg-background px-3 pt-2 shrink-0" style={{ paddingBottom: "max(env(safe-area-inset-bottom, 0px), 12px)" }}>
            <div className="rounded-3xl border border-border/60 bg-card px-3 py-2.5 shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
              ...
              <div className="mt-1 flex items-center justify-between gap-2">
                ...
                {/* Right cluster: Plan pill · mic · send */}
                <div className="flex items-center gap-1.5">
                  <button
                    type="button"
                    className="inline-flex items-center gap-1 rounded-full h-8 px-3 text-xs font-medium text-primary border border-dashed border-border/60 hover:bg-primary/5 transition-colors"
                    aria-label="Mode"
                  >
                    <span>Plan</span>
                  ...
                  <Button
                    size="icon"
                    className="h-9 w-9 shrink-0 rounded-full bg-black text-white hover:bg-black/90 disabled:bg-muted disabled:text-muted-foreground"
                    onClick={() => sendMessage()}
