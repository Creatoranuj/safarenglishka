          ...
          {attachments.map((att, idx) => {
            ...
            return (
              <div key={att.id} className="flex items-center gap-2 p-1.5 bg-muted/30 rounded-md">
                ...
                <Button variant="ghost" size="icon" className="h-11 w-11" disabled={idx === 0} onClick={() => moveUp(idx)}>
                  <ArrowUp className="h-3 w-3" />
                </Button>
                <Button variant="ghost" size="icon" className="h-11 w-11" disabled={idx === attachments.length - 1} onClick={() => moveDown(idx)}>
                  <ArrowDown className="h-3 w-3" />
                </Button>
                <Button variant="ghost" size="icon" className="h-11 w-11 text-destructive" onClick={() => { if (confirm(`Delete "${att.title || att.file_name}"?`)) void deleteAttachment(att.id); }}>
