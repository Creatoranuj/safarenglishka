...
const AdminSmartNotes = lazyWithRetry(() => import("./pages/AdminSmartNotes"));
...
const QueryCacheBoot = () => {
  useEffect(() => {
    ...
    // Install/device tracking — one throttled RPC per device per 24h, fired
    // after auth has had a chance to restore so the row links to the student.
    const installTimer = window.setTimeout(() => {
      void import("./lib/installTracker")
        .then((m) => m.recordAppInstall())
        .catch(() => { /* best-effort */ });
    }, 4000);
    // When useResumeRecovery fires `app:resumed`, refresh only the queries the
    // user can actually be looking at *and* that change per-user. Invalidating
    // everything (the old behaviour) refetched the whole cache — including
    // near-static landing/notice/catalog data — on every app switch, which was
    // a top egress driver.
    const RESUME_STALE_PREFIXES = [
      "profile", "enrollments", "lesson_progress", "user-subscription",
      "notification_reads", "doubts", "messages", "live",
    ];
    const onResumed = () => {
      void queryClient.invalidateQueries({
        ...
        predicate: (q) => {
          ...
          return typeof head === "string" && RESUME_STALE_PREFIXES.includes(head);
        },
      });
    };
              ...
              <BrowserRouter>
                <NavigationHistoryProvider>
                  ...
                  <Routes>
                    ...
                    <Route path="/admin/smart-notes" element={<AdminRoute element={<AdminSmartNotes />} />} />
