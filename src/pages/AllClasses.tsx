    ...
    <div className="min-h-dvh bg-background nb-smooth-scroll">
      ...
      <div className="p-5 space-y-4">
        {activeTab === "courses" && filteredCourses.map((course) => {
          ...
          const openCourse = () => navigate(`/classes/${course.id}/chapters?from=all-classes`);
          return (
            <div
              key={course.id}
              role="button"
              tabIndex={0}
              aria-label={`${course.title}, ${course.lessonCount} lessons`}
              onClick={openCourse}
              onKeyDown={(e) => {
                if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openCourse(); }
              }}
              className="flex items-center gap-4 p-4 bg-card border border-border rounded-xl shadow-sm cursor-pointer hover:shadow-md transition-shadow focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            >
