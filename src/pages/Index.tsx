...
const Navigation = memo(({ isAuthenticated }: { isAuthenticated: boolean }) => {
  ...
  const desktopNav = (
    <>
      <Link to="/courses" className={linkCls}>{t("nav.courses")}</Link>
      <Link to="/books" className={linkCls}>{t("nav.resources")}</Link>
      <a href="#why-choose-us" onClick={scrollToWhy} className={linkCls}>{t("nav.about")}</a>
    </>
  );
    ...
    >
      <div className="container mx-auto max-w-7xl px-6 lg:px-10 h-16 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2.5">
          <img src={logo} alt="Safar English" className="h-8 w-8 rounded-md" loading="eager" />
          <span
        ...
        <div className="hidden md:flex items-center gap-3">
          {isAuthenticated ? (
            <Link to="/dashboard" onClick={() => { void selectionHaptic(); }}>
              <Button className="h-10 px-5 rounded-md text-sm font-medium active:scale-[0.97] transition-transform duration-150">{t("nav.dashboard")}</Button>
            </Link>
          ) : (
            <>
              <Link to="/login" onClick={() => { void selectionHaptic(); }} className={linkCls}>{t("auth.login")}</Link>
              <Link to="/signup" onClick={() => { void tapHaptic("light"); }}>
                <Button className="h-10 px-5 rounded-md text-sm font-medium active:scale-[0.97] transition-transform duration-150">{t("auth.signupFree")}</Button>
              </Link>
            </>
          )}
        </div>
          ...
          <SheetContent side="right" className="w-72">
            ...
            <div className="flex flex-col gap-1 mt-8">
              <Link to="/courses" onClick={() => { void selectionHaptic(); setMobileOpen(false); }}>
                <Button variant="ghost" className="h-11 w-full justify-start">{t("nav.courses")}</Button>
              </Link>
              <Link to="/books" onClick={() => { void selectionHaptic(); setMobileOpen(false); }}>
                <Button variant="ghost" className="h-11 w-full justify-start">{t("nav.resources")}</Button>
              </Link>
              <a href="#why-choose-us" onClick={(e) => { void selectionHaptic(); setMobileOpen(false); scrollToWhy(e); }}>
                <Button variant="ghost" className="h-11 w-full justify-start">{t("nav.about")}</Button>
              </a>
              <div className="h-px bg-border my-4" />
              {isAuthenticated ? (
                <Link to="/dashboard" onClick={() => { void tapHaptic("light"); setMobileOpen(false); }}>
                  <Button className="h-11 w-full">{t("nav.dashboard")}</Button>
                </Link>
              ) : (
                <>
                  <Link to="/login" onClick={() => { void selectionHaptic(); setMobileOpen(false); }}>
                    <Button variant="outline" className="h-11 w-full">{t("auth.login")}</Button>
                  </Link>
                  <Link to="/signup" onClick={() => { void tapHaptic("light"); setMobileOpen(false); }}>
                    <Button className="h-11 w-full mt-2">{t("auth.signupFree")}</Button>
                  </Link>
                </>
              )}
            </div>
