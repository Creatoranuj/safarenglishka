...
import { Loader2, Trash2, Upload, Link as LinkIcon, FileText } from "lucide-react";
...
export default function AdminStudyMaterials() {
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => setCourses((data ?? []) as { id: number; title: string }[]));
  }, []);
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => setChapters((data ?? []) as { id: string; title: string; course_id: number }[]));
    setChapterId("");
  }, [courseId]);
        ...
        <section className="space-y-2">
          ...
          {!courseId ? (
            ...
            <ul className="space-y-2">
              {materials.map((m) => (
                <li key={m.id} className="flex items-start gap-3 p-3 rounded-xl border bg-card">
