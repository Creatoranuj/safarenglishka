...
import { openRazorpayCheckout, formatRazorpayError, sanitizeContact, UPI_FIRST_CHECKOUT_CONFIG, type RazorpaySuccessResponse } from "../utils/razorpay";
  ...
  const handleRazorpayPayment = async (opts?: { forceWeb?: boolean }) => {
    ...
    const sharedOpts = {
      ...
      prefill: {
        name: user.fullName || "",
        email: user.email || "",
        // Contact prefill skips an extra step in the UPI collect flow.
        ...(sanitizeContact(profile?.mobile) ? { contact: sanitizeContact(profile?.mobile)! } : {}),
      },
