...
import appLogo from "../assets/branding/safar-englishka-logo.webp";
...
const APK_FALLBACK_URL = "https://github.com/MrAnujBabu/Safar-Englishka-Install/releases/latest/download/SafarEnglish.apk";
const GITHUB_LATEST_API =
  "https://api.github.com/repos/MrAnujBabu/Safar-Englishka-Install/releases/latest";
const APK_CACHE_KEY = "nb:latest_apk:v1";
const APK_CACHE_TTL_MS = 6 * 60 * 60 * 1000; // 6h

type ApkInfo = { url: string; tag: string | null };

function readCachedApk(): ApkInfo | null {
  try {
    const raw = localStorage.getItem(APK_CACHE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as { ts: number; url: string; tag: string | null };
    if (!parsed?.url || Date.now() - parsed.ts > APK_CACHE_TTL_MS) return null;
    return { url: parsed.url, tag: parsed.tag ?? null };
  } catch { return null; }
}
...
async function fetchLatestApk(signal: AbortSignal): Promise<ApkInfo | null> {
  try {
    ...
    const asset = Array.isArray(json?.assets)
      ? json.assets.find((a: any) => typeof a?.browser_download_url === "string" && /\.apk$/i.test(a.name || ""))
      : null;
    const url: string | undefined = asset?.browser_download_url;
    if (!url || !url.startsWith("https://")) return null;
    return { url, tag: typeof json?.tag_name === "string" ? json.tag_name : null };
  } catch { return null; }
}
...
const Install = () => {
  ...
  const [apk, setApk] = useState<ApkInfo>(() =>
    readCachedApk() ?? { url: APK_FALLBACK_URL, tag: null }
  );
  ...
  const handleDirectDownload = () => {
    // Trigger cross-origin APK download without leaving the install page.
    // GitHub release assets return Content-Disposition: attachment, so an
    // invisible iframe starts the download while the user stays here.
    ...
    toast.success("APK download started! Check your notifications.");
    // Clean up after the download handshake has had time to start.
    setTimeout(() => {
    ...
    <div className="min-h-dvh bg-background">
      ...
      <div className="container mx-auto px-4 max-w-2xl pb-16">
        ...
        <div className="relative overflow-hidden rounded-2xl mt-5 mb-6 bg-gradient-to-br from-primary/20 via-primary/10 to-background border border-primary/20 px-6 pt-8 pb-6 text-center">
          ...
          />
          <h1 className="text-2xl font-bold text-foreground">Install Safar English App</h1>
          ...
          <div className="flex items-center gap-3 p-4 rounded-xl bg-green-500/10 border border-green-500/30 mb-5">
            ...
            <div>
              <p className="font-semibold text-green-600 dark:text-green-400 text-sm">
                You're already using the Safar English app! 🎉
              </p>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-border bg-card overflow-hidden">
              ...
              <div className="p-5 space-y-4">
                ...
                <p className="text-[11px] text-muted-foreground text-center">
                  Android 7.0+ · ~23 MB{apk.tag ? ` · Latest: ${apk.tag}` : ""}
                </p>
          ...
          <div className="rounded-2xl border border-border bg-card overflow-hidden">
            ...
            <div className="p-5 space-y-3">
              ...
              <StepCard
                ...
                description="The Safar English app icon will appear on your home screen!"
              />
