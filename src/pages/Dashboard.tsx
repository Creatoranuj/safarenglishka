...
const Dashboard = () => {
  ...
  // "Continue where you left" follows the batch chosen in the dropdown.
  const activeCourse = useMemo(() => {
    if (myCourses.length === 0) return null;
    if (selectedBatch) {
      const match = myCourses.find((c) => String(c.id) === String(selectedBatch.id));
              ...
              <div className="flex items-center justify-between gap-3 bg-gradient-to-r from-primary to-accent rounded-xl px-4 py-3 shadow-md">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <span className="text-sm font-medium truncate text-primary-foreground">
                    Install the Safar English app for a better experience
                  </span>
                </div>
            ...
            {activeCourse ? (
              ...
              >
                <div className="flex flex-col sm:flex-row">
                  <div className="w-full sm:w-48 sm:h-auto sm:self-stretch bg-muted relative overflow-hidden flex-shrink-0">
                    <SmartImage 
                      src={activeCourse.thumbnailUrl || activeCourse.imageUrl || "/placeholder.svg"} 
                      alt={activeCourse.title}
                      width={600}
                      height={320}
                      fallbackSrc={coursePlaceholder}
                      className="block w-full h-auto sm:absolute sm:inset-0 sm:h-full sm:object-cover object-center"
                    />

                    <div className="absolute inset-0 bg-black/30 flex items-center justify-center opacity-0 [@media(hover:hover)]:group-hover:opacity-100 transition-opacity">
                  ...
                  <div className="p-4 flex-1 flex flex-col justify-center gap-1.5 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <Badge variant="secondary" className="text-xs shrink-0">
                        Class {activeCourse.grade || "General"}
                      </Badge>
                      <span className="text-xs text-muted-foreground truncate">Continue where you left</span>
                    </div>
                    <h3 className="text-base sm:text-lg font-bold text-foreground line-clamp-1">{activeCourse.title}</h3>
                    ...
                    <Button size="sm" className="mt-1 h-11 w-fit px-4 bg-accent text-accent-foreground [@media(hover:hover)]:hover:bg-accent/90">
