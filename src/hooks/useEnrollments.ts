...
// Bandwidth: this hook mounts on several screens and used to refetch the
// enrollments⨝courses join on every mount *and* on every `user` object
// identity change (auth token refresh). A tiny module-level TTL cache +
// in-flight dedupe collapses those into one request per minute per user.
const ENROLLMENTS_TTL_MS = 60_000;
type EnrollmentsSnapshot = { userId: string; at: number; rows: EnrollmentWithCourse[] };
let enrollmentsCache: EnrollmentsSnapshot | null = null;
let enrollmentsInFlight: { userId: string; promise: Promise<EnrollmentWithCourse[]> } | null = null;

export const invalidateEnrollmentsCache = () => {
...
export const useEnrollments = () => {
  ...
  const fetchEnrollments = useCallback(async (force = false) => {
    if (!userId) {
    ...
    const apply = (rows: EnrollmentWithCourse[]) => {
      if (!aliveRef.current) return;
      setEnrollments(rows);
      setEnrolledCourseIds(rows.filter((e) => e.status === "active").map((e) => e.courseId));
      setLoading(false);
    };
    ...
    if (
      enrollmentsCache &&
      enrollmentsCache.userId === userId &&
      Date.now() - enrollmentsCache.at < ENROLLMENTS_TTL_MS
    ) {
      apply(enrollmentsCache.rows);
      return;
    }

    if (enrollmentsInFlight && enrollmentsInFlight.userId === userId) {
      try {
        apply(await enrollmentsInFlight.promise);
      } catch {
        /* the owning caller reports the error */
      }
      return;
    }
    ...
    try {
      ...
      const load = (async (): Promise<EnrollmentWithCourse[]> => {
      ...
      const formatted: EnrollmentWithCourse[] = settled
        ...
        enrollmentsCache = { userId, at: Date.now(), rows: formatted };
        return formatted;
      })();

      enrollmentsInFlight = { userId, promise: load };
