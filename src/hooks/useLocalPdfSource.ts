 ...
 * Handles both storage tiers:
 * - `web-indexeddb:` local_path  → bytes live in `downloadFileDB`.
 * - Native `Dir:path` local_path → bytes live on the device filesystem.
 * This bypasses the fragile `URL.createObjectURL` path that produced
 * "Unexpected server response (0) while retrieving PDF blob:..." on APK
 * whenever the WebView evicted its blob cache (visibilitychange, resume).
 */
async function readNbDownloadAsBlob(id: number): Promise<Blob | null> {
  try {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (nbId) {
          blob = (await readNbDownloadAsBlob(Number(nbId))) ?? undefined;
          if (!blob) {
            const rec = await getDownload(Number(nbId));
            if (rec?.url && typeof navigator !== "undefined" && navigator.onLine !== false) {
