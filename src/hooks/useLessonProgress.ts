...
export function useLessonProgress(
  ...
  // Signature of the last payload actually written — used to suppress
  // no-op upserts while the player is paused/idle.
  ...
  const writeNow = useCallback(async () => {
    ...
    // Bandwidth: a paused/idle player still ticks, which used to produce an
    // identical upsert every 5s. Skip the write when nothing actually moved
    // since the last flush (updated_at is excluded from the comparison).
    const signature = JSON.stringify([
      payload.watched_seconds,
      payload.last_position_seconds,
      payload.completed,
      payload.watched_intervals,
    ]);
