...
import { openRazorpayCheckout, formatRazorpayError, sanitizeContact, UPI_FIRST_CHECKOUT_CONFIG, type RazorpaySuccessResponse } from "./razorpay";
import {
...
export const openSubscriptionCheckout = async (
  ...
  const sharedOpts = {
    ...
    prefill: { name: user.name, email: user.email, ...(sanitizeContact(user.contact) ? { contact: sanitizeContact(user.contact)! } : {}) },
