...
export const openNativeRazorpayCheckout = async (
  ...
  const nativeMethod = {
    // UPI first: the Android SDK renders the sheet in key order, so keeping
    // `upi` at the top puts GPay / PhonePe / Paytm above cards.
    upi: true,
    ...(method ?? {}),
    ...
    netbanking: true,
    wallet: true,
    // Keep the sheet compact so UPI stays above the fold.
    emi: false,
    paylater: false,
  };
  ...
  try {
    ...
    addBreadcrumb('payment', 'razorpay:open', {
      ...
      // Exact method map we handed to the native SDK — if UPI ever stops
      // rendering in the APK, this tells us instantly whether the client
      // requested it or Razorpay dropped it server-side.
      methods_enabled: Object.entries(payload.method as Record<string, boolean>)
        .filter(([, v]) => v).map(([k]) => k).join(','),
      has_contact_prefill: Boolean((options.prefill as { contact?: string } | undefined)?.contact),
    });
