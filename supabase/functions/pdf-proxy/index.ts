...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (kind === "url") {
      ...
      // Pick up admin-managed hosts (cached 60s) before the allow-list check.
      await refreshTrustedPdfHosts();
...
// Admin-managed extension of the static baseline: `public.trusted_hosts`
// rows with category frame/website and enabled = true. Cached in-memory for
// 60s so the hot path never waits on Postgres. The SSRF guards below still
// apply to every dynamic host — this only widens *which hostnames* are
// acceptable, never the protocol/port/IP rules.
let dynamicHosts: string[] = [];
let dynamicHostsFetchedAt = 0;
const DYNAMIC_HOSTS_TTL_MS = 60_000;

export async function refreshTrustedPdfHosts(): Promise<void> {
  if (!adminClient) return;
  if (Date.now() - dynamicHostsFetchedAt < DYNAMIC_HOSTS_TTL_MS) return;
  try {
    const { data, error } = await adminClient
      .from("trusted_hosts")
      .select("host")
      .eq("enabled", true)
      .in("category", ["frame", "website"]);
    if (error) throw error;
    dynamicHosts = (data ?? [])
      .map((r) => String(r.host || "").trim().toLowerCase().replace(/^\*\./, ""))
      .filter((h) => /^[a-z0-9.-]+\.[a-z]{2,}$/.test(h));
    dynamicHostsFetchedAt = Date.now();
  } catch (err) {
    console.warn("[pdf-proxy] trusted_hosts refresh failed", (err as Error).message);
  }
}

function matchesDynamicHost(host: string): boolean {
  return dynamicHosts.some((h) => host === h || host.endsWith("." + h));
}
