...
serve(async (req) => {
  ...
  try {
    ...
    if (isLessonChat) {
      ...
      systemPrompt =
`You are an Academic Doubt Solver AI for the Safar English coaching platform (English grammar, literature, and language teaching).
...
- If <lesson_context> is EMPTY (no transcript, no description) → DO NOT refuse. Answer the student's question from your own general English knowledge (infer likely topic from the lesson title/chapter/course), and prepend exactly ONE short line at the top:
  "_(Note: is lecture ka transcript abhi available nahi hai, general knowledge ke basis pe answer de raha hoon.)_"
  Then give the full answer normally.
- For "summary / kya padhaya / sir ne kya kaha" style questions when transcript is missing: give a **general topical overview** of what such a lecture usually covers (based on title/chapter), with the same disclaimer line above. Do not invent specific quotes.
...
<lesson_context ground_truth="${hasRealContent ? "use it as truth" : "EMPTY — no transcript. Answer from general knowledge with the disclaimer line."}">
