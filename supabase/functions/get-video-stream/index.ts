...
// ─── Stream URL host allowlist (SSRF guard) ───
// Third-party instances (Piped/Invidious) are untrusted input: a malicious or
// compromised instance could return a URL pointing at internal/metadata hosts.
// Only YouTube's own CDN hosts (and the instance origin itself, for proxied
// playback) may ever be handed back to the client.
const YT_CDN_HOST_RE = /(^|\.)googlevideo\.com$|(^|\.)youtube\.com$|(^|\.)ytimg\.com$|(^|\.)ggpht\.com$/;

function isAllowedStreamHost(url: string, instanceOrigin?: string): boolean {
  try {
    const u = new URL(url);
    if (u.protocol !== "https:") return false;
    if (YT_CDN_HOST_RE.test(u.hostname)) return true;
    // Piped/Invidious proxy their own origin (/latest_version?id=...&local=true)
    if (instanceOrigin) {
      try { if (u.origin === new URL(instanceOrigin).origin) return true; } catch { return false; }
    }
    return false;
  } catch {
    return false;
...
function filterStreams(streams: StreamInfo[], source: string): StreamInfo[] {
  // source is like "piped:https://api.piped.private.coffee" or "invidious:inv.nadeko.net"
  let instanceOrigin: string | undefined;
  const m = source.match(/^(piped|invidious):(.+)$/);
  if (m) {
    const raw = m[2];
    instanceOrigin = raw.startsWith("http") ? raw : `https://${raw}`;
  }
  return streams.filter((s) => isAllowedStreamHost(s.url, instanceOrigin));
}
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // SSRF guard: drop any stream URL whose host is not YouTube's CDN or the
    // instance origin that produced it. Never proxy untrusted hosts.
    const safe = filterStreams(streams, source);
    if (safe.length === 0) {
      console.error(`[result] ✗ all ${streams.length} streams failed host allowlist (source=${source})`);
      return new Response(JSON.stringify({ error: "Stream extraction failed", details: "No streams from trusted hosts." }), { status: 502, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    if (safe.length < streams.length) {
      console.log(`[allowlist] dropped ${streams.length - safe.length} untrusted-host streams (source=${source})`);
    }

    const sorted = sortStreams(safe);
