...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const prompt = `Tum Safar Sarthi ho — Safar English ka AI research assistant. Tumhe web search results mile hain is topic ke baare mein:
...
Hindi/Hinglish mein likho.`;
    ...
    const aiBody: Record<string, unknown> = {
      ...
      messages: [
        { role: "system", content: "You are Safar Sarthi, the AI research companion for Safar English. Create curated study guides from web search results for Indian students. CRITICAL: Only use information from the provided web sources. Do NOT fabricate facts, page numbers, or references that are not in the sources. Cite every claim with [Source N]. If information is insufficient, state it clearly." },
